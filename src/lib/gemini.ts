 ...
 */
export const generateProblem = async (difficulty: string, excludeTitles: string[] = [], focusCategory?: string): Promise<Problem> => {
  ...
  for (const modelName of modelsToTry) {
    try {
      console.log(`Attempting to generate problem with model: ${modelName}`);
      const model = genAI.getGenerativeModel({ model: modelName });

      const categoryPrompt = focusCategory
        ? `Constraints: The user is weak in "${focusCategory}". Generate a problem SPECIFICALLY about ${focusCategory} to help them practice.`
        : "";
      ...
      const prompt = `
        ...
        - You MUST generate exactly 2 test cases and exactly 1 hint.
        - **CRITICAL**: The hint must directly help the user pass Test Case 1.
  ...
  try {
    ...
    // Throw the original Gemini error to give the user a hint about the API key if applicable
    throw new Error((lastError as Error)?.message || ("Failed to generate a new challenge with both Gemini and Groq."));
  }
};
...
export const analyzeCode = async (
  ...
  for (const modelName of modelsToTry) {
    try {
      console.log(`Attempting code analysis with model: ${modelName}`);
      // ...
      const model = genAI.getGenerativeModel({ model: modelName });
      ...
      console.warn(`Analysis model ${modelName} failed:`, (error as Error).message);
      // Continue to next model
    }
  }
  ...
  return {
    hasError: true,
    message: `Analysis failed. Check API key or connection.`,
    isCorrect: false
  };
};
